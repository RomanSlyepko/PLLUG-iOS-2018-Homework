...
class ArtistsSearchVC: UIViewController {
    ...
    func getArtists(result: ResultsPage){
        ...
        DispatchQueue.main.async {
            ...
            if !(self.searchBar.text?.isEmpty ?? true){
                ...
                if let array = result.resultsPage.results.artist{
                    self.foundArtists = array
                }else{
                    self.foundArtists = []
                }
                
                self.pageManager.totalResults = result.resultsPage.totalEntries
    ...
    func moreArtists(result: ResultsPage){
        DispatchQueue.main.async {
            guard let array = result.resultsPage.results.artist else {return}
            self.foundArtists += array
